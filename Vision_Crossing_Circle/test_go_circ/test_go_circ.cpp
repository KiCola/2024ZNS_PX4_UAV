...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber ocvd_sub = nh.subscribe<px4_command::hough>("px4/hough_sender", 100, ocvd_cb);
    ...
    while(!go_through_circ)
    {
        ...
        if(circ_yz_abs_distance<inaccuracy_tolerance){
            ...
            Uav_x = pos_drone.pose.position.x;
            Uav_y = pos_drone.pose.position.y; 
            float fly_new_height;
            if(circ_coordinates[2] <= fly_height +0.2 && circ_coordinates[2] >= fly_height - 0.2)
            {
                fly_new_height = circ_coordinates[2];
            }
            else
            {
                fly_new_height = fly_height;
            }
            ...
            while(ros::ok())
            {
                ...
                Command_now.pos_sp[1] =  Uav_y ;  //当前y，也可改用发布的圆环y
                Command_now.pos_sp[2] =  fly_new_height; //按发布的圆环高度穿越
                Command_now.yaw_sp = 0;
                Command_pub.publish(Command_now);
            ...
            while(ros::ok())
            {
                ...
                comid++;
                Command_now.sub_mode = 0; // xy 位置控制模式 z 位置控制模式
                Command_now.pos_sp[0] =  Uav_x ;//依旧是飞机当前x
                Command_now.pos_sp[1] =  circ_coordinates[1]; //按测得的圆环y
                Command_now.pos_sp[2] =  circ_coordinates[2]; //按测得的圆环h
                Command_now.yaw_sp = 0;
                Command_pub.publish(Command_now);

                abs_distance = find_distance(pos_drone.pose.position.y, pos_drone.pose.position.z, )
            }
...
void calculate_circ_xyz(){
	...
	float circ_dy=-theoretical_distance_to_circ*(target_circ.x-cx_fisheye)/fx_fisheye;
	float circ_dz=-theoretical_distance_to_circ*(target_circ.y-cy_fisheye)/fy_fisheye;
