...
int main(int argc, char **argv)
{
    ...
    //filter
    nh.param<int>("open_filter", open_filter, 0);
    nh.param<float>("filter_dy_range", filter_dy_range, 0.2);
    nh.param<float>("filter_dz_range", filter_dz_range, 0.2);
    nh.param<float>("ddd_y_range",ddd_y_range, 0.12);
    nh.param<float>("ddd_z_range",ddd_z_range, 0.12);
    ...
    last_judgement=0;
    circ_adjust_times=0;
    filter_circle_pos_init(open_filter);
    int test;
    cin >> test;
    ...
    while(!go_through_circ)
    {
        ...
        if(target_circ.x<0||target_circ.y<0||target_circ.y>target_circ.rows||target_circ.x>target_circ.cols)
        {
            if(last_judgement&&theoretical_distance_to_circ<near_min_distance){
                ...
                while(ros::ok())
                {
                    ...
                    Command_now.pos_sp[1] =  Uav_y ;  //当前y，也可改用发布的圆环y
                    Command_now.pos_sp[2] =  fly_height; 
                    Command_now.yaw_sp = 0;
                    Command_pub.publish(Command_now);
        ...
        if(theoretical_distance_to_circ>=max_detection_distance||theoretical_distance_to_circ<=near_min_distance){
            if(last_judgement&&theoretical_distance_to_circ<=near_min_distance){
                ...
                while(ros::ok())
                {
                    ...
                    Command_now.sub_mode = 0; // xy 位置控制模式 z 位置控制模式
                    Command_now.pos_sp[0] =  Uav_x + 2 * min_detection_distance ;//min给0.5？
                    Command_now.pos_sp[1] =  now_circ_coordinates[1] ;  //当前y，也可改用发布的圆环y
                    Command_now.pos_sp[2] =  now_circ_coordinates[2]; //按发布的圆环高度穿越
                    Command_now.yaw_sp = 0;
                    Command_pub.publish(Command_now);
                    
                    cout << "pos:" << pos_drone.pose.position.x << "   " << pos_drone.pose.position.y << endl;
                    cout << "target position:" << Uav_x + 2 * min_detection_distance << "  " << now_circ_coordinates[1] << "  " << now_circ_coordinates[2] << endl;
                    abs_distance = find_distance(pos_drone.pose.position.x, pos_drone.pose.position.y, Uav_x + 2 * min_detection_distance, now_circ_coordinates[1]);
                    if(abs_distance <= 0.1)
                    {
        ...
        calculate_circ_xyz();
        filter_queue_update(open_filter);
        ...
        if(circ_yz_abs_distance<inaccuracy_tolerance){
            ...
            while(ros::ok())
            {
                ...
                abs_distance = find_distance(pos_drone.pose.position.x, pos_drone.pose.position.y, Uav_x +0.25, Uav_y);
                cout<<"now x:"<<pos_drone.pose.position.x<<"\n    now y:"<<pos_drone.pose.position.y<<endl;
                cout<<"now z:"<<pos_drone.pose.position.z;
                cout<<"     target x:"<< Uav_x+ 0.25<<"  target y:"<< now_circ_coordinates[1]<<"  target z:"<<now_circ_coordinates[2]<<endl;
            ...
            if(abs(now_circ_coordinates[1]-pos_drone.pose.position.y)>largest_stride||abs(now_circ_coordinates[2]-pos_drone.pose.position.z)>largest_stride||now_circ_coordinates[2]<min_circ_height){
                ...
                Command_now.pos_sp[1] =  now_circ_coordinates[1]; //按测得的圆环y
                Command_now.pos_sp[2] =  (now_circ_coordinates[2]>min_circ_height?now_circ_coordinates[2]:min_circ_height); //按测得的圆环h
                Command_now.yaw_sp = 0;
                Command_pub.publish(Command_now);
                rate.sleep();
            }
                ...
                while(one_step_time<one_step_patience)
                {
                    ...
                    Command_now.yaw_sp = 0;
                    Command_pub.publish(Command_now);
                    abs_distance = find_distance(pos_drone.pose.position.y, pos_drone.pose.position.z, now_circ_coordinates[1], now_circ_coordinates[2]);
                    cout <<"taking steps to centre"<<endl;
                    cout << "dy: " << now_circ_coordinates[1] - pos_drone.pose.position.y << "  dz: " << now_circ_coordinates[2] - pos_drone.pose.position.z << endl;
