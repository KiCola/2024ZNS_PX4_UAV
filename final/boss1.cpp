...
float circle_back_pos;
float no_target_Land;
...
int main(int argc, char **argv)
{
    ...
    nh.param<float>("inaccuracy_tolerance", inaccuracy_tolerance, 0.05);
    nh.param<int>("one_step_patience", one_step_patience, 8);
    nh.param<float>("largest_stride", largest_stride, 0.10);
    nh.param<float>("min_circ_height", min_circ_height, 0.65);
    nh.param<float>("max_circ_height", max_circ_height, 1.05);

    nh.param<float>("circle_back_pos",circle_back_pos, 2.63);
    nh.param<float>("no_target_Land",no_target_Land, 4.2);
    ...
    while(!go_through_circ)
    {
        ...
        if(target_circ.x<0||target_circ.y<0||target_circ.y>target_circ.rows||target_circ.x>target_circ.cols)
        {
            if(last_judgement&&theoretical_distance_to_circ<near_min_distance){
                ...
                while(ros::ok())
                {
                    ...
                    comid++;
                    Command_now.sub_mode = 0; // xy 位置控制模式 z 位置控制模式
                    Command_now.pos_sp[0] =  circle_back_pos ;//min给0.5？
                    Command_now.pos_sp[1] =  0.00 ;  //当前y，也可改用发布的圆环y
                    Command_now.pos_sp[2] =  fly_height; 
                    Command_now.yaw_sp = 0;
                    Command_pub.publish(Command_now);
                    ...
                    abs_distance = find_distance(pos_drone.pose.position.x, pos_drone.pose.position.y, Uav_x + 2 * min_detection_distance, Uav_y);
                    if(abs_distance <= 0.1)
                    {
        ...
        if(theoretical_distance_to_circ>=max_detection_distance||theoretical_distance_to_circ<=near_min_distance){
            if(last_judgement&&theoretical_distance_to_circ<=near_min_distance){
                go_through_circ=1;
                ...
                while(ros::ok())
                {
                    ...
                    Command_now.pos_sp[0] =  circle_back_pos ;//min给0.5？
                    Command_now.pos_sp[1] =  0.00 ;  //当前y，也可改用发布的圆环y
                    Command_now.pos_sp[2] =  now_circ_coordinates[2]; //按发布的圆环高度穿越
                    Command_now.yaw_sp = 0;
                    Command_pub.publish(Command_now);
                    
                    abs_distance = find_distance(pos_drone.pose.position.x, pos_drone.pose.position.y, circle_back_pos, 0.00);
                    if(abs_distance <= 0.1)
                    {
                        cout << "achieved crossing circle task!!! 2222222" << endl;
                        ROS_INFO("***************************************************crossed circle***********************************************");
                        break;
                    }
                    else
                    {
                        ROS_INFO("Task2 ...");
                        cout << "-------------------------------------------------------------------------------------" << "\n";
                        cout << "approaching circle 222222222222222222222... comid:" << comid << "\n";
                        cout << "abs_distance: " << abs_distance << "\n";
                        cout << "pos_x: " << pos_drone.pose.position.x << " pos_y: " << pos_drone.pose.position.y << "\n";
                        cout << "now_height:  " << pos_drone.pose.position.z << endl; 
                        cout << "target position:" << circle_back_pos << "  " << 0.00 << "  " << now_circ_coordinates[2] << endl;
                        cout << "-------------------------------------------------------------------------------------" << "\n";      
                    }
        ...
        if(circ_yz_abs_distance<inaccuracy_tolerance){
            ...
            while(ros::ok())
            {
                ...
                abs_distance = find_distance(pos_drone.pose.position.x, pos_drone.pose.position.y, Uav_x +0.25, 0.00);
                if(abs_distance <= 0.1)
                {
                    break;
                }
                else
                {
                    ROS_INFO("Task2 ...");
                    cout << "-------------------------------------------------------------------------------------" << "\n";
                    cout << "approaching circle .step forward ... 3333333333333333333333... comid:" << comid << "\n";
                    cout << "abs_distance: " << abs_distance << "\n";
                    cout << "pos_x: " << pos_drone.pose.position.x << " pos_y: " << pos_drone.pose.position.y << "\n";
                    cout << "now_height:  " << pos_drone.pose.position.z << endl; 
                    cout << "target x:"<< Uav_x+ 0.25<<"  target y:"<< 0.00<<"  target z:"<<now_circ_coordinates[2]<<endl;
                    cout << "-------------------------------------------------------------------------------------" << "\n";   
                }
                rate.sleep();
            }
            ...
            now_circ_coordinates[1] = circ_coordinates[1];
            now_circ_coordinates[2] = circ_coordinates[2];
            if(abs(now_circ_coordinates[1])>largest_stride||abs(now_circ_coordinates[2]-pos_drone.pose.position.z)>largest_stride||now_circ_coordinates[2]<min_circ_height||now_circ_coordinates[2]>max_circ_height){
                ROS_WARN("large movement,wrong indeed,now take one step");
                ros::spinOnce();
                if(now_circ_coordinates[2]<min_circ_height){
                    now_circ_coordinates[2]=min_circ_height;
                }
                if(now_circ_coordinates[2]>max_circ_height){
                    now_circ_coordinates[2]=max_circ_height;
                }
                Command_now.command = Move_ENU;     //世界坐标系下移动
                Command_now.comid = comid;
                comid++;
                ...
                Command_now.pos_sp[1] =  0.00; //按测得的圆环y
                Command_now.pos_sp[2] =  now_circ_coordinates[2]; //按测得的圆环h
                Command_now.yaw_sp = 0;
                Command_pub.publish(Command_now);
                rate.sleep();
            }
                ...
                while(one_step_time<one_step_patience)
                {
                    ...
                    Command_now.yaw_sp = 0;
                    Command_pub.publish(Command_now);
                    abs_distance = find_distance(pos_drone.pose.position.y, pos_drone.pose.position.z, 0.00, now_circ_coordinates[2]);
                    ...
                    {
                        ROS_INFO("Task2 :  one-step ...    ");cout << "one_step_time:" << one_step_time;
                        cout << "-------------------------------------------------------------------------------------" << "\n";
                        cout << "taking steps to centre"<<endl;
                        cout << "dy: " << 0.00 - pos_drone.pose.position.y << "  dz: " << now_circ_coordinates[2] - pos_drone.pose.position.z << endl;
    ...
    //now_circ_coordinates[2] = fly_height;

    ros::spinOnce();
    cout << "*****************************************************************" << endl;
    cout << "UAV has crossed the circle!:" << presetCir_x << "," << presetCir_y << ", " << now_circ_coordinates[2] << endl;
    ...
    while(ros::ok())
    {
		...
		Command_now.pos_sp[1] = Uav_y + 0.3 // ENU frame
		Command_now.pos_sp[2] = fly_height;
		Command_now.yaw_sp = 0;
		Command_pub.publish(Command_now);
		abs_distance = find_distance(pos_drone.pose.position.x, pos_drone.pose.position.y, initial_target_x, initial_target_y);
    ...
    while (ros::ok())
    {
        ...
        if(flag == 2 && abs(distance_cy) > 0.1)
        {
            ...
            ROS_WARN("collision_avoidance!!!!!!!!!!!!!!!!!!!!!!!!!\n");
        } //紧急避障
        else if(flag == 0){
            track(target_x, target_y);
            ROS_INFO("Normal tracking!!!!");
        }
        else
        {
        ...
        Command_now.pos_sp[2] =  now_circ_coordinates[2];
        Command_now.yaw_sp = 0 ;
        Command_pub.publish(Command_now);
    ...
    Uav_x = pos_drone.pose.position.x;
    Uav_y = pos_drone.pose.position.y;
    float travelling_height = now_circ_coordinates[2];
    ...
    while(ros::ok){
        ...
        if(spot_target)
        {
            ...
            if(abs_distance < arrival_thresh)
            {
                ...
                else
                {
                    ...
                    Command_now.pos_sp[2] = travelling_height;//zzr:这里感觉可以降低高度
                    Command_now.yaw_sp = 0;
                    Command_pub.publish(Command_now);
                }
            }
                ...
                cout << "pos_drone:" << pos_drone.pose.position.x << " " << pos_drone.pose.position.y <<" " << pos_drone.pose.position.z << endl;
        ...
        if(no_target_time>no_target_patience)
        {
            ...
            while(ros::ok())
            {
                if(Uav_x < no_target_Land)
                {
                    Command_now.command = Move_ENU;
                    Command_now.comid = comid;
                    comid++;
                    Command_now.sub_mode = 0;
                    Command_now.pos_sp[0] = Uav_x + 0.2;
                    Command_now.pos_sp[1] = Uav_y; // ENU frame
                    Command_now.pos_sp[2] = travelling_height;//zzr:这里感觉可以降低高度
                    Command_now.yaw_sp = 0;
                    Command_pub.publish(Command_now);
                    cout << "approaching xy:" << Uav_x + 0.1 << " , " << Uav_y  <<endl;
                    
                    ros::spinOnce();
                    abs_distance = find_distance(pos_drone.pose.position.x, pos_drone.pose.position.y, Uav_x+0.2, Uav_y);
                    if(abs_distance < 0.1)
                    {break;}
                    rate.sleep();
                }
                ...
                {
                    ROS_WARN("Dangerous! UAV is approaching the wall!!! execute safe Land");
                    Command_now.command = Land;
                    while (ros::ok())
                    {
                        Command_pub.publish(Command_now);
                ...
                /*else
                {
                    ...
                    Command_now.vel_sp[0] = 0;
                    Command_now.vel_sp[1] = 0; // ENU frame
                    Command_now.pos_sp[2] = now_circ_coordinates[2];//zzr:这里感觉可以降低高度
                    Command_now.yaw_sp = 0;
                    for(int i=0; i<10; i++)
                        Command_pub.publish(Command_now);
                    ROS_WARN("Uav is dangourous since its pos approaches search_distance!");
...
void calculate_circ_xyz(){
	...
	float circ_dy=0.00 - pos_drone.pose.position.y;
    float circ_dz=-theoretical_distance_to_circ*(target_circ.y-target_circ.cy)/target_circ.fy -0.02;
